...
Calculates total cM matching DNA.
...
from VCF_Comparer_configV2 import (
    AUTO_SNP_MIN,
    CHROM_TRUE_SIZE,
    CHROMOSOMES,
    DNA_FILES_PATH,
    ...
    try:
        with open(file_path, "r", encoding="utf-8", errors="ignore") as f:
            for _ in range(20):
                ...
                if not line:
                    break
                if line.startswith(("##fileformat=VCF", "#CHROM")):
                    return True
    except Exception:
        return False
    return False
...
def _normalize_dna_dataframe(df, no_call_val):
    ...
    # Keep rows where allele is "A", "C", "G", or "T" (case-insensitive)
    keep_vals = {"A", "C", "G", "T"}
    mask = df["allele1"].astype(str).str.upper().isin(keep_vals) | df["allele2"].astype(
        str
    ).str.upper().isin(keep_vals)
    df = df[mask].copy()
...
def _parse_vcf_file(file_path, requested_individuals, no_call_val):
    ...
    for sample_col in individuals_to_load:
        ...
        def get_alleles(genotypes, row_indices=row_indices, num_snps=num_snps, allele_mat=allele_mat, no_call_val=no_call_val, max_alts=max_alts):
        ...
        if df.empty:
            ...
            no_calls = (
                (df["allele1"] == no_call_val) | (df["allele2"] == no_call_val)
            ).sum()
            total = len(df)
            pc_no_calls = (no_calls / total) * 100 if total > 0 else 0
            if pc_no_calls > PC_NO_CALLS_ALLOWED:
                print(
                    f"Dropping individual '{sample_col}' due to high no-call percentage: {pc_no_calls:.2f}%"
                )
                continue
...
def agnostic_load_individual_dna(ind, files_path, no_call_val, return_error=False):
    ...
    for filname in candidates:
        ...
        try:
            ...
            pc_no_calls = (no_calls / total) * 100 if total > 0 else 0
            if pc_no_calls > PC_NO_CALLS_ALLOWED:
                last_error = f"Individual '{ind}' dropped from '{filname}' due to high no-call percentage: {pc_no_calls:.2f}%"
                continue
...
def find_segments(
    m_vals, p_vals, c_vals, cutoff, snp_min, chrom, mm_dist, is_fir=False, no_call_mask=None, max_contig_nc=None
):
    ...
    Args:
        ...
        no_call_mask (np.array): Boolean mask where True indicates a no-call position.
        max_contig_nc (int): Maximum allowed contiguous no-calls in a segment.
    """
    ...
    for s, e in zip(starts, ends):
        n = e - s + 1
        if n >= snp_min:
            # Enforce contiguous no-call limit if configured
            if no_call_mask is not None and max_contig_nc is not None:
                seg_nc = no_call_mask[s:e+1]
                if np.any(seg_nc):
                    # Find maximum run of True in seg_nc
                    diff_nc = np.diff(seg_nc.astype(int), prepend=0, append=0)
                    nc_starts = np.where(diff_nc == 1)[0]
                    nc_ends = np.where(diff_nc == -1)[0]
                    if len(nc_starts) > 0:
                        max_run = np.max(nc_ends - nc_starts)
                        if max_run > max_contig_nc:
                            continue
...
def analyze_chromosome(
    ...
    for p1, p2 in match_pairs:
        ...
        # Any no-call (0) in either individual at this position
        nc_mask = (a1_1 == 0) | (a2_1 == 0) | (a1_2 == 0) | (a2_2 == 0)
        ...
        # 2: full match
        m_vals[full] = 2
        # Handle no-calls based on config
        if config.get("NO_CALLS_MATCHES", False):
            # Treat no-calls as matches initially
            m_vals[nc_mask] = 2
            # BREAK segments if contiguous no-calls are too long
            max_nc = config.get("NO_CALLS_CONTIG_MAX_NO")
            if max_nc is not None:
                # Find runs of no-calls
                diff_nc = np.diff(nc_mask.astype(int), prepend=0, append=0)
                nc_starts = np.where(diff_nc == 1)[0]
                nc_ends = np.where(diff_nc == -1)[0]
                for s_nc, e_nc in zip(nc_starts, nc_ends):
                    if (e_nc - s_nc) > max_nc:
                        # Treat this run as a mismatch to break any segment
                        m_vals[s_nc:e_nc] = 0
        else:
            # Treat no-calls as no-matches (mismatches)
            m_vals[nc_mask] = 0
...
def main():
    ...
    config = {
        ...
        "HIR_SNP_MIN": HIR_CUTOFF * 30 if AUTO_SNP_MIN else HIR_SNP_MIN,
        "FIR_SNP_MIN": FIR_CUTOFF * 30 if AUTO_SNP_MIN else FIR_SNP_MIN,
        "MM_DIST": MM_DIST,
        "NO_CALL": NO_CALL,
        "NO_CALLS_MATCHES": NO_CALLS_MATCHES,
        "NO_CALLS_CONTIG_MAX_NO": NO_CALLS_CONTIG_MAX_NO,
    ...
    print("\nData saved to " + os.path.join(WORKING_DIRECTORY, f"{EXCEL_FILE_NAME}.csv"))
